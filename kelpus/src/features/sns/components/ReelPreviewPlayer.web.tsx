/**
 * 웹 전용: react-native-view-shot은 웹 미지원
 * captureCurrentFrame은 항상 reject → shareService가 imageDataUrl 없이 텍스트 공유로 폴백
 */
import React, {
  forwardRef,
  useEffect,
...
export const ReelPreviewPlayer = forwardRef<ReelPreviewPlayerHandle, Props>(
  ({frames, onDone, compact = false}, ref) => {
    ...
    // 웹에서는 캡처 미지원 — shareService가 catch해서 텍스트 공유로 폴백
    useImperativeHandle(ref, () => ({
      captureCurrentFrame(): Promise<string> {
        return Promise.reject(new Error('웹에서 캡처 미지원'));
      },
    }));
